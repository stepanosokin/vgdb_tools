        ...
        for item in vgjson['data']:
            if 'type' in item.keys():
                if item['type'] == 'seismic_dataset' and 'id' in item.keys():
                    update_seismic_link(item['id'], filepath.replace('vgdb.vgjson', ''), vgdb_pass)
                    print(item['id'], filepath.replace('vgdb.vgjson', ''))


def update_seismic_link(id, link, vgdb_pass):
    with psycopg2.connect(f"dbname=vgdb host=192.168.117.3 user=s.osokin password={vgdb_pass}") as conn:
        cur = conn.cursor()
        cur.execute(f"""
        update dm.seismic_datasets set link = '{link}' where dataset_id = {id};
        update dm.links set link = '{link}' where link_id in (select link_id from dm.links_to_datasets where dataset_id = {id});
        """)
