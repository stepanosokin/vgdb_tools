        ...
        for filename in fnmatch.filter(files, '*.xls*'):
            ...
            for nrow in range(nrows):
                ...
                if str(df.iloc[nrow, excel_col_nums['block_num']]) != 'nan' and len(str(df.iloc[nrow, excel_col_nums['block_num']])) > 0 and str(df.iloc[nrow, excel_col_nums['y_s']]).replace(',', '').replace('.', '').isdigit() and str(df.iloc[nrow, excel_col_nums['y_s']]) != 'nan':
                    ...
                    # if str(df.iloc[nrow, -1]) == 'nan' and not changed_colnums_flag:
                    #     changed_colnums_flag = True
                    #     for key in excel_col_nums.keys():
                    #         if excel_col_nums[key] < 0:
                    #             excel_col_nums[key] = excel_col_nums[key] - 1
                    #             pass
                    field_vals = [
                    ...
                    # if excel_file[-4:] == '.xls':
                    #     if type(df.iloc[nrow, -5]) == float and str(df.iloc[nrow, -5]) != 'nan':
                    #         excel_col_nums['reserves_predicted_resources'] = -2
                    #         excel_col_nums['exp_protocol'] = -1
                    #         excel_col_nums['usage_type'] = -2
                    #         excel_col_nums['lend_type'] = -1
                    #         excel_col_nums['planned_terms_conditions'] = -1
                    #         pass
                    #         field_vals = [
                    #             df.iloc[nrow, excel_col_nums['resource_type']],
                    #             df.iloc[nrow, excel_col_nums['name']],
